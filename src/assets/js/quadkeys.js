/* PIERO: sostituito operatore << con lshift
...
var Quadkey = (function (exports) {
  ...
  function mapSize(detail) {
    return lshift(256, parseInt(detail, 10));
  }
   ...
   */
  exports.sibling = function (quadkey, _direction) {
    var _map = {
      'left': Direction.Left,
      'right': Direction.Right,
      'up': Direction.Up,
      'up-left': Direction.Up,
      'up-right': Direction.Up,
      'down': Direction.Down,
      'down-left': Direction.Down,
      'down-right': Direction.Down
    };
    var direction = _map[_direction];

    _direction = _direction.split("-");
    if (_direction[1]) {
      quadkey = keyTranslate(quadkey, quadkey.length - 1, direction);
      direction = _map[_direction[1]];
      return keyTranslate(quadkey, quadkey.length - 1, direction);
    } else
      return keyTranslate(quadkey, quadkey.length - 1, direction);
  };

  exports.siblings = function (quadkey, type) {
    var dirs;
    if (type && type == 'all')
      dirs = ['left', 'right', 'up', 'down', 'up-left', 'up-right', 'down-left', 'down-right'];
    else
      dirs = ['left', 'right', 'up', 'down'];
    var sibs = [];
    for (var d in dirs) {
      sibs.push(exports.sibling(quadkey, dirs[d]));
    }
    return sibs;
  };
